...
SearchRequest.prototype._toBer = function (ber) {
  ...
  if (typeof filterBer.startSequence !== 'function') {
    // When migrating all of the filter code into `@ldapjs/filter`, we changed the
    // way the `.toBer` function works. The original implementation relied on
    // side effects. The new implementation does not.
    // TODO: remove this once all filters have been updated to new style (2022-06-12)
    for (let i = 0; i < filterBer._buf.length; i++) {
      ber.writeByte(filterBer._buf[i])
    }
  } else {
    // The side effect based `.toBer` has already written itself to te passed
    // in BER instance. Thus, we need to make sure we update our local pointer
    // correctly.
    ber = filterBer
  }
